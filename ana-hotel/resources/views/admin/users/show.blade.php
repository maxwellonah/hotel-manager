        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                <div class="flex items-center justify-between mb-6">
                    ...
                    <div class="space-x-2">
                        <a href="{{ route('admin.users.edit', $user) }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-md hover:bg-indigo-700">Edit User</a>
                        <a href="{{ route('admin.users.index') }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50">Back to Users</a>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 mb-8">
                    ...
                    <div>
                        ...
                        <div class="text-gray-900">{{ $user->phone ?? 'N/A' }}</div>
                    </div>
                    <div>
                        <div class="text-sm text-gray-500">Role</div>
                        <div class="text-gray-900">{{ ucfirst($user->role) }}</div>
                    </div>
                    <div>
                        <div class="text-sm text-gray-500">Joined</div>
                        <div class="text-gray-900">{{ optional($user->created_at)->format('M j, Y') }}</div>
                    </div>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($bookings as $booking)
                                @php
                                    $hasCompletedPayment = $booking->payments && $booking->payments->contains(function ($p) {
                                        return $p->status === \App\Models\Payment::STATUS_COMPLETED;
                                    });
                                    $canAccept = $hasCompletedPayment && $booking->status !== 'checked_in' && $booking->payment_status !== 'paid';
                                @endphp
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ optional($booking->check_in)->format('Y-m-d') }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ optional($booking->check_out)->format('Y-m-d') }}</td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ optional($booking->payment_confirmed_at)->format('Y-m-d H:i') ?? '—' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        @if($canAccept)
                                            <form action="{{ route('bookings.accept-payment', $booking->id) }}" method="POST" class="inline" onsubmit="return confirm('Confirm you want to accept this payment and record today as the paid date?');">
                                                @csrf
